...
class ManagerController @Autowired constructor(private val service: ManagerServiceSpec): ManagerControllerSpec {
    override fun saveName(jwt: Jwt, name: String): ResponseEntity<String> {
        val responseBody = this.service.saveName(name.substring(0, name.length-1), jwt.subject.substring(6))
        return ResponseEntity.status(HttpStatus.CREATED).body(responseBody)
    }

    override fun createSnippet(jwt: Jwt, snippetContent: CreateSnippet): ResponseEntity<Output> {
        try{
            val responseBody = service.createSnippet(snippetContent,  jwt.subject.substring(6))
            return ResponseEntity.status(HttpStatus.CREATED).body(responseBody)
    ...
    override fun getSnippetDescriptors(jwt: Jwt): ResponseEntity<Output> {
        try{
            val responseBody: SnippetListDto = service.getSnippetDescriptors(jwt.subject.substring(6))
            return ResponseEntity.ok(responseBody)
        } catch(e: HttpClientErrorException){
            return ResponseEntity.status(e.statusCode).body(ErrorOutput(e.message!!))
        } catch(e: NotFoundException){
